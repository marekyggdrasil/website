...
Back in 2015 [Gregory Maxwell introduced the scheme of the confidential transactions](https://elementsproject.org/features/confidential-transactions/investigation) based on Pedersen commitments allowing a very private way of storing transaction amounts. Let us jump right into this concept and imagine that each participant forms own part of the transaction.
...
where $$ G $$ generates the blinding factors indicated by $$ b $$ and $$ H $$ generates the values or amounts indicated by $$ v $$. Having vanishing difference of two commitment proving the non-inflation can be done by verifying a [Schnorr signature](/2021/05/25/schnorr-signature/) with blinding factors as private keys. Such signature can only be valid if generated by $$ G $$ which is true only if the difference of values cancels out. This can be verified in Python using our regular ECC code as follows
...
where `Gx` and `Gy` are the coordinates of the generator point on the curve and `n` is the prime order of the group generated by $$ G $$. To understand the notation of this function please check previous tutorials related to ECC where more explicit explanations are provided.
...
where $$ b G $$ is transaction excess. Valid signature proves *non-inflation* as difference of inputs and outputs vanishes as well as *ownership* as only participant knowing the blinding factor can produce valid signature.
